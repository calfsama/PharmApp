...
class FilterViewController: UIViewController {
    ...
    @objc func tapped() {
        ...
        data?.data(minimumPrice: "\(slider.values.minimum)", maximumPrice: "\(slider.values.maximum)", kind: "String", raiting: "")
...
//        doubleSlider.trackLayer.borderColor = UIColor(red: 0.738, green: 0.741, blue: 1, alpha: 1).cgColor
//        doubleSlider.trackLayer.borderWidth = 0.5
//        doubleSlider.trackTintColor = .white
...
//        doubleSlider.editingDidEndDelegate = self
    ...
    private func makeLabels() {
        for num in stride(from: 1, to: 200, by: 1) {
            labels.append("\(num)")
        }
    }
    ...
    private func setupDoubleSlider() {
        let height: CGFloat = 40.0 //TODO: make this the default height
        let width = view.bounds.width - 38.0
        
        let frame = CGRect(
            x: containerView.bounds.minX - 2.0,
            y: containerView.bounds.midY + 20,
            width: width,
            height: height
        )
...
//        doubleSlider.addTarget(self, action: #selector(printVal(_:)), for: .valueChanged)
        ...
        containerView.addSubview(doubleSlider)
    }
...
////            doubleSlider.centerXAnchor.constraint(equalTo: containerView.centerXAnchor),
//
...
extension FilterViewController: DoubleSliderEditingDidEndDelegate {
...
extension FilterViewController: DoubleSliderLabelDelegate {
